...
layout(set = 2, binding = 1) uniform sampler2D s_combine1;
layout(set = 2, binding = 2) uniform sampler2D s_combine2;
layout(set = 2, binding = 3) uniform sampler2D s_shadow;
...
void main()
{
    ...
    vec4 combine1 = texture(s_combine1, coord);
    vec4 combine2 = texture(s_combine2, coord);
    vec3 position;
    vec2 uv;
    vec3 normal;
    position.x = combine1.x;
    position.y = combine1.y;
    position.z = combine1.z;
    uv.x = combine1.w;
    uv.y = combine2.x;
    if (length(uv) == 0)
    {
